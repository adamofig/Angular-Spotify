...
export class ApiRequestService {
  ...
  private getAuthorizationCoded(): string {
    ...
    //btoa() aplica un algoritmo de codificación como solicita Spotify https://beta.developer.spotify.com/documentation/general/guides/authorization-guide/#client-credentials-flow
    return "basic " + window.btoa(environment.clientID + ":" + environment.clientSecret);
  }

  public async loginSpotify(): Promise<any> {
    let urlForToken = "https://accounts.spotify.com/api/token";
    ...
    let response: any = await this.httpClient.post(urlForToken, this.bodyGrantTypeFormData, { headers: headers }).toPromise();
  ...
  public async searchInSpotify(artist: string): Promise<any> {
    ...
    let urlToQuery = `${this.spotifyApiUrl}search?q=${artist}&type=artist`;
    let response = await this.httpClient.get(urlToQuery, { headers: headers }).toPromise();
